...
contract SimpleEcommerce {
    ...
    function placeOrder(uint256[] calldata productIds, uint256[] calldata quantities) external payable {
        ...
        address seller = address(0); // Initialize seller address
        ...
        for (uint256 i = 0; i < productIds.length; i++) {
            ...
            if (i == 0) {
                ...
                require(p.seller == seller, "All products must be from the same seller");
            }
    ...
    function debugGetAllBuyerOrders(address buyer) external view returns (uint256[] memory) {
        return buyerOrders[buyer];
    }

    function debugGetAllSellerOrders(address seller) external view returns (uint256[] memory) {
